...
const NotificationDropdown = () => {
    ...
    const getNotifications = async () => {
        let payloadUrl = `user/notifications`
        let method = "GET"
    ...
    const getNotificationsCount = async () => {
        let payloadUrl = `user/notifications/count`
        let method = "GET"
        
        const res = await ApiService.fetchData(payloadUrl,method)
        if( res && process.env.REACT_APP_API_SC_CODE.includes(res.status_code)){
            let data = res.results
            setNotificationsCount(Number(data))
        }
    }
    ...
    return (
        <React.Fragment>
            <Dropdown className="topbar-head-dropdown ms-1 header-item">
                <Dropdown.Toggle type="button" tag="button" className="btn btn-icon btn-topbar btn-ghost-secondary rounded-circle shadow-none btn-none">
                ...
                <Dropdown.Menu className="dropdown-menu-lg dropdown-menu-end p-0">
                    <div className="dropdown-head bg-primary bg-pattern rounded-top">
                        ...
                        <div className="px-2 pt-2">
                            <ul className="nav nav-tabs-custom dropdown-tabs nav-tabs-custom">
                                <li className="nav-item" role="presentation"> <a className={`nav-link ${activeTab == 1 ? "active" : ""}`} onClick={() => toggleTab("1")}> All (4) </a></li>
                                <li className="nav-item" role="presentation"> <a className={`nav-link ${activeTab == 2 ? "active" : ""}`} onClick={() => toggleTab("2")}> Messages </a></li>
                                <li className="nav-item" role="presentation"> <a className={`nav-link ${activeTab == 2 ? "active" : ""}`} onClick={() => toggleTab("3")}> Alerts </a></li>
                            </ul>
                        </div>
